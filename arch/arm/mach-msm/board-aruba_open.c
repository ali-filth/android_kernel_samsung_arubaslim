...
#define SENSE_00 39
#define SENSE_01 37
#endif	// #if defined (CONFIG_MACH_ARUBA_CTC) 
...
#endif
#ifdef CONFIG_INPUT_BMA2x2_ACC_ALERT_INT
#if defined(CONFIG_MACH_ARUBA_OPEN) &&!defined(CONFIG_MACH_DELOS_OPEN)
	if (board_hw_revision >= 6)
#endif
...
static int gp2a_power(bool on)
{
	...
	if (l1 == NULL) {
		l1 = regulator_get(NULL, "ldo01");
		if (IS_ERR(l1)) {
			printk(KERN_DEBUG "%s: vreg_get(%s) failed (%ld)\n"
			, __func__, "ldo01", PTR_ERR(l1));
			return 0;
		}
	...
	gpio_tlmm_config(GPIO_CFG(29 , 0 ,
				GPIO_CFG_INPUT,
				GPIO_CFG_PULL_UP,
				GPIO_CFG_2MA),
				GPIO_CFG_ENABLE);
...
#if defined(CONFIG_MACH_ARUBA_CTC) || defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_ARUBASLIM_OPEN)\
|| defined(CONFIG_MACH_KYLE_PLUS)|| defined(CONFIG_MACH_KYLEPLUS_OPEN) || defined(CONFIG_MACH_DELOS_OPEN)
...
static unsigned int kp_col_gpios[] = {36, 39};			// input pu
#else
...
static struct bq24157_platform_data bq24157_pdata = {
	...
	.VMchrg = BQ24157_68m_VMCHG_1050,
	.chg_curr_ta = BQ24157_68m_CURR_1050,
...
#if defined(CONFIG_MACH_DELOS_OPEN)
#define GPIO_CHARGER_SCL 	78 //share with fuelgauge
#define GPIO_CHARGER_SDA 	79 //share with fuelgauge
#define GPIO_CHARGER_STAT	9 //PM_GPIO9 //PM8209 is not supported that controlled in MSM
#else
...
#elif defined(CONFIG_MACH_ARUBA_CTC) || defined(CONFIG_MACH_ARUBA_DUOS_CTC)
...
#if defined(CONFIG_CHARGER_BQ24157) && defined(CONFIG_MACH_DELOS_OPEN)
	{
		I2C_BOARD_INFO("bq24157", 0x6A),
		.platform_data = &bq24157_pdata,
		//.irq = MSM_GPIO_TO_INT(GPIO_CHARGER_STAT), //PM8209 is not supported that controlled in MSM
	},
#endif
};
#endif
...
static int bluetooth_power(int on)
{
	...
	if (on) {
		...
		if(board_hw_revision <= 6) //for just aruba open old bt hw
		{
			printk(KERN_DEBUG "bluetooth on config bt_host_wake for old hw \n");
			gpio_tlmm_config(GPIO_CFG(GPIO_BT_HOST_WAKE_GPIO34, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
		}
#elif defined(CONFIG_MACH_KYLEPLUS_OPEN) || defined(CONFIG_MACH_DELOS_OPEN)
		if(board_hw_revision < 1) //for kyleplus and Delos open old bt hw
		{
			printk(KERN_DEBUG "bluetooth on config bt_host_wake for old hw \n");
			gpio_tlmm_config(GPIO_CFG(GPIO_BT_HOST_WAKE_GPIO34, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
		}
		...
		if(board_hw_revision <= 6) //for just aruba open old bt hw
		{
			printk(KERN_DEBUG "bluetooth off config bt_host_wake for old hw \n");
			gpio_tlmm_config(GPIO_CFG(GPIO_BT_HOST_WAKE_GPIO34, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
		}
#elif defined(CONFIG_MACH_KYLEPLUS_OPEN) || defined(CONFIG_MACH_DELOS_OPEN)
		if(board_hw_revision < 1) //for kyleplus  and Delos open old bt hw
		{
